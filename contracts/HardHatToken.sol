...
contract HardHatToken {
   ...
   * 
   * The `external` modifier makes a 
   * function *only* from *outside* the contract
   */
  function transfer(address to, uint256 amount) external {
    // Check if the transaction sender enough tokens.
    // If `require`'s first argument evalutes to `false` then the
  ...
  /**
   * Read only function to retrieve 
   * the token balance of a given account.
   * 
   * The `view` modifier indicates that it doesn't 
